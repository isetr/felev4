(function()
{
   ...
   return Concurrency.Delay(function()
   {
    ...
    return Concurrency.Bind(x,function(a)
    {
     return Concurrency.FromContinuations(function(ok,err,cc)
     {
      ...
      {
       return waiting[0]?(waiting[0]=false,cc(OperationCanceledException.New.call(this,Concurrency.noneCT()))):null;
 ...
 Arrays.countBy=function(f,a)
 {
  ...
  for(i=0,$1=Arrays.length(a)-1;i<=$1;i++){
   k=f(a[i]);
   d.ContainsKey(k)?d.set_Item(k,d.get_Item(k)+1):(v=keys.push(k),void 0,d.Add(k,1));
  }
  f$1=function(k$1)
  {
 ...
 Arrays.groupBy=function(f,a)
 {
  ...
  for(i=0,$1=Arrays.length(a)-1;i<=$1;i++){
   c=a[i];
   k=f(c);
   d.ContainsKey(k)?(v=d.get_Item(k).push(c),void 0):(v$1=keys.push(k),void 0,d.Add(k,[c]));
  }
  f$1=function(k$1)
  {
   return[k$1,d.get_Item(k$1)];
 ...
 Arrays.tryPick=function(f,arr)
 {
  ...
  while(i<arr.length?res==null:false)
   {
    m=f(arr[i]);
    (m!=null?m.$==1:false)?void(res=m):null;
    i=i+1;
   }
  return res;
 };
 ...
 CancellationTokenSource=WebSharper.CancellationTokenSource=Runtime.Class({
  CancelAfter:function(delay)
  {
   var $this,o;
   $this=this;
   !this.c?(o=this.pending,o==null?null:Global.clearTimeout(o.$0),this.pending={
    $:1,
    $0:Global.setTimeout(function()
    {
     $this.Cancel$1();
 ...
 Delegate.Remove=function(source,value)
 {
  ...
  for(i=Arrays.length(sourceInv)-1,$2=0;i>=$2;i--){
   ...
   (!found?Runtime.DelegateEqual(it,value):false)?void(found=true):(v=resInv.unshift(it),void 0);
  }
 ...
 Dictionary=Collections.Dictionary=Runtime.Class({
  ...
  add:function(k,v)
  {
   ...
   }(d))?DictionaryUtil.alreadyAdded():null,this.count=this.count+1,(v$1=d.push({
    K:k,
    V:v
   }),void 0)):(this.count=this.count+1,void(this.data[h]=new Global.Array({
    K:k,
    V:v
   })));
  },
  set:function(k,v)
  {
   ...
   },function(a)
   {
    ...
    K:k,
    V:v
   })):(this.count=this.count+1,void(this.data[h]=new Global.Array({
    K:k,
    V:v
   })));
  },
 ...
 HashSet=Collections.HashSet=Runtime.Class({
  ...
  SymmetricExceptWith:function(xs)
  {
   ...
   try
   {
    while(e.MoveNext())
     {
      item=e.Current();
      this.Contains(item)?(v=this.Remove(item),void 0):(v$1=this.Add(item),void 0);
     }
   }
   finally
   {
  ...
  RemoveWhere:function(cond)
  {
   ...
   for(i=0,$1=all.length-1;i<=$1;i++){
    item=all[i];
    cond(item)?this.Remove(item)?void(res=res+1):null:null;
   }
   return res;
  },
 ...
 List.exists2=function(p,x1,x2)
 {
  ...
  (!e?l1.$==1?true:l2.$==1:false)?List.badLengths():null;
  return e;
 };
 ...
 List.forall2=function(p,x1,x2)
 {
  ...
  (a?l1.$==1?true:l2.$==1:false)?List.badLengths():null;
  return a;
 };
 ...
 List.map3=function(f,x1,x2,x3)
 {
  ...
  ((l1.$==1?true:l2.$==1)?true:l3.$==1)?List.badLengths():null;
  return res;
 };
 ...
 Task=WebSharper.Task=Runtime.Class({
  ...
  ContinueWith:function(func,ct)
  {
   ...
   this.get_IsCompleted()?res.StartContinuation():(v=this.continuations.push(res),void 0);
   return res;
  },
  ...
  OnCompleted:function(cont)
  {
   ...
   if(this.get_IsCompleted())
    ...
    {
     Unchecked.Equals(this.get_Status(),0)?this.Start():null;
